...
export default function Gifts({ clothingSize, shoeSize, toySuggestion, giftNote, pixKey }: GiftsProps) {
  const [selectedCota, setSelectedCota] = useState<Cota | null>(null);
  const [qrDataUrl, setQrDataUrl] = useState("");
  const [copiaColaString, setCopiaColaString] = useState("");
  ...
  const cotas: Cota[] = [
    {
      ...
      amount: "R$ 40,00",
      purpose: "Ajuda na compra de brinquedos interativos e descobertas novas para o Hazael.",
      icon: "🥕",
    ...
    },
    {
      ...
      amount: "R$ 75,00",
      purpose: "Uma ajuda especial para os looks moderninhos do pequeno aniversariante.",
      icon: "🐥",
    ...
    },
    {
      ...
      amount: "R$ 110,00",
      purpose: "Contribui para sapatos confortaveis e presentes cheios de personalidade.",
      icon: "🦆",
    ...
    },
    {
      ...
      purpose: "Valor livre para fortalecer o cofrinho das proximas aventuras do Hazael.",
      icon: "🌪️",
      character: "Taz Baby",
    },
  ];
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Erro na geracao de dados Pix:", err);
    }
  }, [selectedCota, pixKey]);
    ...
    <div id="gifts-section" className="w-full max-w-md mx-auto space-y-6">
      <div className="bg-white border-4 border-[#FEF08A] rounded-[40px] p-6 sm:p-8">
        ...
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="bg-[#FEF08A]/20 p-3 sm:p-4 rounded-2xl flex flex-col items-center justify-center border border-[#FEF08A]/40">
            <span className="text-2xl mb-1">👕</span>
            <span className="font-bold text-[#854D0E] uppercase tracking-tight text-[9px]">Roupas</span>
            <span className="font-fredoka font-black text-sm text-[#854D0E] mt-1">{clothingSize || "2 anos"}</span>
          </div>

          <div className="bg-[#FEF08A]/20 p-3 sm:p-4 rounded-2xl flex flex-col items-center justify-center border border-[#FEF08A]/40">
            <span className="text-2xl mb-1">👟</span>
            <span className="font-bold text-[#854D0E] uppercase tracking-tight text-[9px]">Sapato</span>
            <span className="font-fredoka font-black text-sm text-[#854D0E] mt-1">{shoeSize || "22"}</span>
          </div>

          <div className="bg-[#FEF08A]/20 p-3 sm:p-4 rounded-2xl flex flex-col items-center justify-center border border-[#FEF08A]/40">
            <span className="text-2xl mb-1">🧩</span>
            <span className="font-bold text-[#854D0E] uppercase tracking-tight text-[9px]">Brinquedos</span>
            <span className="font-fredoka font-black text-sm text-[#854D0E] mt-1">{toySuggestion || "Interativos"}</span>
          </div>
        </div>

        <div className="mt-4 rounded-[28px] border border-[#FCD34D]/70 bg-[#FFF7D6] px-4 py-3 text-left text-xs font-semibold leading-relaxed text-[#854D0E]">
          <span className="mb-1 block font-fredoka text-sm font-black text-[#B45309]">Dica carinhosa</span>
          {giftNote}
        </div>
      </div>
      ...
      <div className="bg-[#FEF08A] rounded-[40px] border-4 border-white p-6 sm:p-8 flex flex-col shadow-md">
        <div className="text-center mb-5">
          ...
          <p className="text-[10px] text-[#854D0E] leading-tight opacity-80 mt-1 max-w-[280px] mx-auto">
            Contribua via PIX com os presentes e as proximas aventuras do Hazael.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2.5">
          {cotas.map((cota) => (
            <div
              key={cota.id}
              className="bg-white/80 rounded-3xl p-4 flex items-center justify-between gap-3 shadow-xs hover:scale-[1.01] active:scale-[0.99] transition-all"
            >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="pix-modal">
          <div className="bg-white rounded-[40px] border-4 border-[#FEF08A] p-6 max-w-sm w-full shadow-2xl relative animate-float">
            ...
            <div className="text-center font-sans">
              ...
              <h4 className="font-fredoka font-black text-lg text-[#854D0E] mb-0.5">{selectedCota.title}</h4>
              ...
              <div className="bg-[#FEF08A]/10 border border-[#FEF08A]/30 rounded-2xl p-3.5 mb-4 text-xs text-[#854D0E] leading-normal text-left">
                ...
                {selectedCota.amount !== "Qualquer valor" && (
                  <p className="mt-2 font-bold text-center text-sm text-[#854D0E]">
                    Sugestao de valor: {selectedCota.amount}
                  </p>
                )}
                <div className="mt-1 text-[9px] text-[#854D0E]/70 border-t border-[#FEF08A]/20 pt-1">
                  <strong>Beneficiaria:</strong> Raiane Oliveira Araujo <br />
                ...
                <div className="bg-white p-2.5 rounded-2xl shadow-xs border border-gray-100 w-36 h-36 flex items-center justify-center">
                  {qrDataUrl ? (
                    ...
                    <div className="flex flex-col items-center justify-center space-y-2 text-gray-400">
                      ...
                      <span className="text-[9px]">Gerando codigo...</span>
                    </div>
                  )}
                </div>
              ...
              <div className="bg-gray-100 rounded-2xl p-2.5 flex flex-col gap-1.5 text-[11px] text-left mb-4">
                ...
                <div className="bg-white rounded-lg p-2 select-all font-mono text-[9px] break-all border border-gray-150 max-h-[55px] overflow-y-auto leading-normal text-gray-700">
                  {copiaColaString || "Carregando codigo..."}
                </div>
                <button
                  ...
                  className={`py-2 px-3 rounded-xl font-bold font-sans tracking-wide uppercase transition-colors text-[10px] cursor-pointer w-full text-center ${
                    copied ? "bg-emerald-500 text-white" : "bg-[#EA580C] hover:bg-[#d94e08] text-white"
                  }`}
                >
              ...
              <p className="text-[10px] text-gray-500 leading-relaxed font-sans max-w-[250px] mx-auto">
                Abra o app do seu banco, entre em Pix, escolha copia e cola ou escaneie o QR Code acima.
              </p>
            </div>
          </div>
        </div>
